...
echo "=== [6] Initialize Kubernetes Control Plane ==="

kubeadm init \
--apiserver-advertise-address=172.31.39.154 \
--pod-network-cidr=10.244.0.0/16 \
--upload-certs

echo "=== [7] Configure kubectl for user ==="

mkdir -p $HOME/.kube
cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
chown $(id -u):$(id -g) $HOME/.kube/config

echo "=== [8] Install CNI (Cilium recommended) ==="
